...
class ElfinderConnector:
    ...
    def __init__(self, opts):
        ...
        for o in opts['roots']:
            # print(opts, 44444444444)
            try:
                volume = instantiate_driver(o)
                # import ipdb; ipdb.set_trace()
            except Exception as e:
                self._mountErrors.append(e.__str__())
                continue
    ...
    def execute(self, cmd, **kwargs):
        ...
        for arg, req in self.commandArgsList(cmd).items():
            if req and (arg not in kwargs or not kwargs[arg]):
    ...
    def _get(self, target):
        ...
        **Command**: Return file contents. This method should not be invoked 
        ...
        try:
            content = volume.get_contents(target).decode()
        except Exception as e:
