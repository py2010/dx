...
from ..exceptions import ElfinderErrorMessages, FileNotFoundError, DirNotFoundError, PermissionDeniedError, NamedError, NotAnImageError
...
class ElfinderVolumeDriver(object):
    ...
    def mount(self, opts):
        ...
        self._defaults = {
            'read': self._options['defaults'].get('read', 1),
            # 'write': self._options['defaults'].get('write', 0),
            # 'locked': False,
            'hidden': False
        }
        self._defaults['locked'] = opts.get('ftp_readonly',
                                            int(not self._options['defaults'].get('write', 0))
                                            )
        self._defaults['write'] = int(not self._defaults.get('locked', 1))
        # print(self._defaults, 44555)
        # root attributes
        ...
        self._root_name = self._options['alias'] or self._basename(self._root)
        ...
        if ('read' not in root or not root['read']) and ('write' not in root and not root['write']):
    ...
    def unmount(self):
        ...
        """
        if self._mounted:
            sftpstorage = self._options.get('storage')
            if sftpstorage:
                sftpstorage._close()
            self._mounted = False
    ...
    def options(self, hash_):
        ...
        return {
            ...
            # 'tmbUrl': self._options['tmbURL'],  # 不显示图片缩略图
    ...
    def mkfile(self, hash_dst, name):
        ...
        if not self._attr(self._join_path(path, name), 'write', dir_['write']):
            raise PermissionDeniedError
    ...
    def rename(self, hash_, name):
        ...
        try:
            ret = self._rename(path, dir_, name)  # 重命名
        except:
    ...
    def encode(self, path):
        ...
        if path:
            ...
            hash_ = b64encode(hash_).translate(bytes.maketrans(b'+/=', b'-_.'))  # 替换字符
    ...
    def decode(self, hash_):
        ...
        if hash_.startswith(self.id()):
            ...
            h = h.encode('ascii').translate(bytes.maketrans(b'-_.', b'+/='))  # 替换字符
            # put cut = at the end
            h += b"=" * ((4 - len(h) % 4) % 4)
            h = b64decode(h)
    ...
    def stat(self, path):
        ...
        if stat_cache is None or root_cache != self._root:
            ...
            if path == self._root:
                ...
                if 'phash' not in stat or not stat['phash']:
                    stat['phash'] = self.encode(self._dirname(path))

            if 'size' not in stat or (not stat['size'] and stat['mime'] == 'directory'):
                stat['size'] = 'unknown'

            stat['read'] = int(self._attr(path, 'read', stat.get('read')))
            stat['write'] = int(self._attr(path, 'write', stat.get('write')))
            # print(stat.get('locked', 0), 7777)
            stat['locked'] = stat.get('locked',
                                      int(not stat.get('write', self._defaults.get('locked', 1)))
                                      )
            # print(stat.get('locked', 0), 7777)
            ...
            if stat['read'] and not self._is_hidden(stat):
                ...
                if stat['mime'] == 'directory':  # handle directories
                    ...
                    if 'tmb' not in stat and self._can_create_tmb(path, stat):
                        stat['tmb'] = self._get_tmb(stat['target'] if 'target' in stat else path, stat)
                    if 'dim' not in stat and stat['mime'].startswith('image'):
                        try:
    ...
    def _attr(self, path, attr, val=False):
        ...
        return val or self._defaults[attr]
    ...
    def _closest_by_attr(self, path, attr, val):
        ...
        if (attr in stat and stat[attr] == val) or (attr not in stat and val == False):
            return path
    ...
    def _search(self, path, q):
        ...
        for p in self._get_cached_dir(path):
            ...
            if q in name:
                ...
                if self._options['URL'] and 'url' not in stat:
                    stat['url'] = self._options['URL'] + p[len(self._root) + 1:].replace(self._separator, '/')
                result.append(stat)

            if stat['mime'] == 'directory' and stat['read'] and 'alias' not in stat:
    ...
    def remove(self, path, force=False):
        ...
        if not force and stat.get('locked', 0):
    ...
    def _checkArchivers(self):
        ...
        if 'archiveMimes' in self._options and self._options['archiveMimes']:
            for mime in self._archivers['create']:
                if mime not in self._options['archiveMimes']:
