...
from ..exceptions import NotAnImageError, ElfinderErrorMessages
...
class ElfinderVolumeStorage(ElfinderVolumeDriver):
    ...
    def mount(self, opts):
        ...
        if 'storage' not in opts:
            if 'storageClass' not in opts:
        ...
        self._options['path'] = '.'
        if ('URL' not in opts or not opts['URL']):
            self._options['URL'] = opts['storage'].url(self._root)

        if 'alias' not in opts or not opts['alias']:
    ...
    def _dimensions(self, path):
        ...
        exception.
        """
        if self._options['storage'].size(path) > 1024 * 1024 * 5:
            # 大图不获取长/宽, 因为文件在远程后端资产
            return '?x?'
        try:
    ...
    def _mimetype(self, path):
        ...
        """
        # print(path, '===========================', mimetypes.types_map[os.path.splitext(path)[1]])
        ...
        if re.search(r'^\./proc/', path) or re.search(r'^\./sys/', path):  # handler /proc /path
            if file_name in self._files:  # handler is files
                try:
                    ...
                    fp.close()
                    # mime = mimetypes.types_map[os.path.splitext(file_name)[1]]
                except:
                    mime = "application/empty"
                return mime
    ...
    def _fopen(self, path, mode='rb'):
        """
        注意, 是从远程sftp服务器读取文件到堡垒机
        判断mime类型时, 只读取文件开头10字节,
        如果是其它读取所有内容的操作, 将很耗性能.
        """
        # print('sftp打开路径:', path)
    ...
    def _move(self, source, target_dir, name, rename=0):
        """
        剪切/移动，先复制，然后再删除旧文件
        """
        ...
        try:
            ...
            if stat['mime'] == 'directory':
                ...
                for p in self._get_cached_dir(source):
                    self._move(p, target, self._basename(p))
                self._rmdir(source)
            else:
    ...
    def _rename(self, source, target_dir, name):
        """
        重命名
        """
        try:
            target = self._join_path(target_dir, name)
            self._options['storage'].rename(source, target)
        except:
            raise os.error
    ...
    def _local_file_mimetype(self, path, name=''):
        ...
        # mime = mimetypes.types_map[os.path.splitext(path.encode('utf-8'))[1]]
